...
the weekly Brief drives the review. **Forward-looking** and **regenerated** from
current state — distinct from the backward-looking, accreted **Diary**.
_Avoid_: "report" (a report draft is a separate, user-facing deliverable); "diary".

**Diary**:
The agent-curated chronological record of a day's thinking, written to
`workspace/diary/YYYY-MM-DD.md`. **Backward-looking** (what was thought / discussed /
decided) and **accreted** across one or more **Sweeps** of the conversation
transcript — the opposite lifecycle to a **Brief** (forward-looking, regenerated).
One Diary file per day; each Sweep appends to it. Not a curated page-graph member:
excluded from the **Index** and orphan lint (like `briefs/`, `archive/`).
_Avoid_: "journal", "daylog", "log" (the per-turn git history is the operation log);
"brief" (opposite lifecycle and direction).
...
**Web Researcher**:
The sandboxed, SSRF-hardened Python service that performs web fetch and search on
behalf of the primary agent. Runs as a sibling process on the same host (later:
own container); holds no credentials; provides **Research findings** via a
localhost-bound FastAPI endpoint. Distinct from a **Grounding Source**: a Grounding
Source is a persistent, indexed external corpus (Confluence, Jira); the Web
Researcher performs ephemeral, on-demand retrieval with no persistent state.
_Avoid_: "grounding source" (it is not persistent or indexed), "search agent"
(it is deterministic, not LLM-driven — LLM mode is a future phase).

**Research broker**:
The MCP server in `agentic-pa` that bridges the primary agent to the **Web
Researcher**. Exposes the `research_query` tool to the agent; forwards queries to
the Web Researcher endpoint, validates responses through a deterministic airlock,
and returns **Research findings**. Runs in the privileged trust domain alongside
the agenda and presenter MCPs.
_Avoid_: "research agent" (the broker is not an LLM; it is a deterministic proxy
with a validation layer), "web search tool" (the agent calls the broker, not the
web directly).

**Research findings**:
The sanitized, schema-validated output of a **Web Researcher** query — a list of
claims with source URLs, title, and retrieval timestamp — after passing the
deterministic airlock in the **Research broker**. What the primary agent receives;
never raw web content.
_Avoid_: "search results" (raw search results do not cross the trust boundary;
only validated findings do), "grounding" (findings are ephemeral and not indexed
...
- The **Research broker** (in `agentic-pa`) proxies `research_query` calls from the
  primary agent to the **Web Researcher** service, validates the response through a
  deterministic airlock, and returns **Research findings**. The primary agent never
  touches the web; the Web Researcher holds no credentials.
