...
def _soft_probe_researcher(port: int, *, retries: int = 20, delay: float = 0.5) -> None:
...
def _apply_research_mcp(
    ...
    if researcher_url:
        # Preserve the existing command if set (generated by build_opencode_config);
        # only update/add the environment block with the current per-run values.
        research_mcp = mcp.get("research", {})
        research_mcp.setdefault("type", "local")
        research_mcp.setdefault("enabled", True)
        env: dict[str, str] = {"RESEARCHER_URL": researcher_url}
        if researcher_secret:
            env["RESEARCHER_SECRET"] = researcher_secret
