...
- The preflight helper (`EnvSpec` + `preflight_env` + the per-shell hint helpers) is ~165 lines + tests, in a single file (`bootstrap_env.py`). No new dependencies.
...
- `secret=True` is implemented and unit-tested but **not yet used by any of the five call sites** — it is reserved for a future secret spec (e.g. an `OPENCODE_SERVER_PASSWORD` entry). When a spec sets it, the value is masked in *every* stderr path: the OK table row, the parse-failure row, **and** the parse-failure detail (the parser's exception text — which can echo the value — is replaced by `PARSE FAILED (value hidden)`). So a malformed secret cannot leak into a CI log. Non-secret parse failures still show the raw value and the exception text, which aids debugging.
- Both `.cmd` and `.sh` shims are structural-tested: the .cmd tests assert the gated `where py` probe, the .sh tests assert the `command -v "$PY"` probe and remediation message. The actual `errorlevel 1` / `exit 1` behavior is only verifiable on a real Windows/POSIX box. Acceptable trade-off — the structural test catches the regression where someone removes the probe while refactoring the shim, which is the failure mode the test was added for.
