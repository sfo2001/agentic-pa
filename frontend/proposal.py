...
def apply_task_ops(notes_root: Path | str, ops: list[dict], *, now: datetime.date | None = None) -> dict:
    ...
    for op in ops:
        ...
        if name == "reprioritize" and value not in ("A", "B", "C", "D"):
            ...
            continue
        if name == "retickle" and bool(validate_action_dates([f"t:{value}"])):
...
def load_staging(notes_root: Path | str) -> tuple[dict, bool]:
    """Load inbox/_proposal.json or the empty default. Ensures `task_ops` exists
    so callers can append directly. Returns (data, was_lost) where was_lost is
    True when an existing file was unparseable and replaced. Mirrors
    presenter.server._load_staging, kept frontend-side to avoid a circular import
    (presenter already imports frontend.proposal)."""
    stage = Path(notes_root) / "inbox" / "_proposal.json"
...
def stage_task_op(notes_root: Path | str, id: str, op: str, value: str | None) -> dict:
    """Validate and stage a single task mutation into inbox/_proposal.json.

    Same validation as presenter.server.present_task: op in {complete,
    reprioritize(A-D), retickle(YYYY-MM-DD)} and the id: must exist in
    tasks.todo.txt. Append-only; does NOT touch tasks.todo.txt (the confirm
    endpoint applies it under git_lock). Caller is responsible for holding
    git_lock so a concurrent confirm cannot clobber the staging file.
    """
    root = Path(notes_root)
    ...
    if op == "retickle" and (not value or bool(validate_action_dates([f"t:{value}"]))):
        return {"ok": False, "error": "retickle value must be YYYY-MM-DD"}
    if not re.fullmatch(ID_BARE_PATTERN, id):
        return {"ok": False, "error": "malformed id"}
    ...
    data, was_lost = load_staging(root)
    data["task_ops"].append({"id": id, "op": op, "value": value})
    (root / "inbox").mkdir(parents=True, exist_ok=True)
    _atomic.atomic_write_text(root / "inbox" / "_proposal.json",
                              json.dumps(data, indent=2), encoding="utf-8")
    res: dict = {"ok": True, "staged": {"id": id, "op": op, "value": value}}
    if was_lost:
        res["warning"] = "prior staging was corrupted and replaced"
    return res
